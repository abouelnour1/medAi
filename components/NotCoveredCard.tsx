...
const NotCoveredCard: React.FC<{ medicine: Medicine, t: TFunction, language?: string }> = ({ medicine, t, language = 'ar' }) => {
        ...
        <div className="bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 rounded-lg shadow-sm p-4 animate-fade-in">
            <div className="flex justify-between items-start gap-4">
                <div>
                    ...
                    <p className="text-sm text-light-text-secondary dark:text-dark-muted line-clamp-2" style={{wordBreak:"break-word"}}>{String(medicine['Scientific Name'] || '')}</p>
                </div>
                <div className="flex-shrink-0 flex flex-col items-end gap-1.5">
                    <div className="text-light-text dark:text-dark-text text-lg font-bold whitespace-nowrap">
                        {!isNaN(price) ? `${price.toFixed(2)} ${language === 'ar' ? 'ر.س' : 'SAR'}` : ''}
                    </div>
