...
const ClinicalDataPage: React.FC<Props> = ({ registerNumber, tradeName, scientificName, language, isAdmin, allMedicines, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-light-bg dark:bg-dark-bg flex flex-col" style={{ touchAction: 'pan-y' }}>
      ...
      {/* Content */}
      <div className="flex-grow overflow-y-auto px-4 py-4 pb-[calc(24px+env(safe-area-inset-bottom))] overscroll-none" style={{ touchAction: 'pan-y', WebkitOverflowScrolling: 'touch' } as any}>
        {loading ? (
