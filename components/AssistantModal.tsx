...
const AssistantModal: React.FC<AssistantModalProps> = ({ 
  ...
  // منع ظهور شاشة تسجيل الدخول أثناء التحميل الأولي من Firebase
  if (isAuthLoading) {
      return (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center backdrop-blur-sm">
            <div className="bg-white dark:bg-dark-card p-6 rounded-2xl flex flex-col items-center">
                <div className="w-8 h-8 border-4 border-primary border-t-transparent rounded-full animate-spin mb-3"></div>
                <p className="text-sm font-bold">جاري التحقق من الجلسة...</p>
            </div>
        </div>
      );
  }
      ...
      return (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center animate-fade-in" onClick={() => onSaveAndClose([])}>
            <div className="bg-white dark:bg-dark-card w-full max-w-md p-8 rounded-2xl shadow-2xl text-center space-y-6 m-4" onClick={e => e.stopPropagation()}>
                ...
                <p className="text-sm text-slate-500">{t('loginRequiredForAI')}</p>
                <div className="flex gap-3">
                    <button onClick={() => onSaveAndClose([])} className="flex-1 py-2 bg-slate-100 dark:bg-slate-700 rounded-lg">{t('cancel')}</button>
                    <button onClick={() => { onSaveAndClose([]); window.location.hash = '#settings'; }} className="flex-1 py-2 bg-primary text-white font-bold rounded-lg">{t('login')}</button>
                </div>
            </div>
        </div>
      );
      ...
      <div className="bg-white dark:bg-dark-card w-full max-w-2xl h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden m-2" onClick={e => e.stopPropagation()}>
        <header className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-slate-700">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold">{t('assistantModalTitle')}</h2>
          </div>
          <button onClick={() => onSaveAndClose(chatHistory)} className="p-2 text-slate-400"><ClearIcon/></button>
        </header>
        ...
        <div className="flex-grow p-4 overflow-y-auto space-y-4">
          {chatHistory.map((msg, index) => {
            ...
            return (
                <div key={index} className={`flex items-start gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div className={`max-w-md rounded-2xl p-3 ${msg.role === 'user' ? 'bg-primary text-white rounded-br-none' : 'bg-gray-100 dark:bg-slate-700 rounded-bl-none'}`}>
          ...
          {isLoading && <div className="text-xs text-slate-400 animate-pulse">جاري الكتابة...</div>}
          <div ref={chatEndRef} />
        </div>

        <footer className="p-4 border-t border-gray-200 dark:border-slate-700">
            <form onSubmit={e => {e.preventDefault(); handleSendMessage();}} className="flex items-center gap-2">
                <textarea value={userInput} onChange={(e) => setUserInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendMessage(); } }} placeholder={t('askGeminiPlaceholder')} className="flex-grow p-2 bg-gray-100 dark:bg-slate-700 rounded-lg outline-none resize-none" rows={1} disabled={isLoading} />
                <button type="submit" disabled={isLoading || !userInput} className="px-4 py-2 bg-primary text-white font-bold rounded-lg">{t('ask')}</button>
            </form>
        </footer>
      </div>
    </div>
