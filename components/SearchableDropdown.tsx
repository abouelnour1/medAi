...
interface SearchableDropdownProps {
  ...
  mode?: 'single' | 'multi';
  headerBottom?: number;
  compact?: boolean; // تصغير حجم العناصر
}
          ...
          <div className="p-3 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
            ...
            <div className="relative">
              <input
                ...
                className="w-full h-8 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus:border-primary rounded-xl outline-none transition-all ltr:pl-9 ltr:pr-4 rtl:pr-9 rtl:pl-4 text-xs"
              />
          ...
          <ul className="overflow-y-auto flex-1 no-scrollbar pb-2" role="listbox">
            {mode === 'single' && (
              <li
                ...
                className="px-4 py-1.5 text-[11px] font-bold text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer border-b border-slate-50 dark:border-slate-800"
                role="option"
                aria-selected={value === ''}
              >
                {t('all')}
              ...
              displayedOptions.map(group => {
                ...
                <React.Fragment key={group.label}>
                  ...
                  {group.options.map(option => {
                    ...
                    return (
                        <li key={option} onClick={() => handleSelect(option)} className={`px-3 py-1.5 text-[11px] cursor-pointer flex justify-between items-center transition-colors ${isSelected ? 'bg-primary/5 font-bold text-primary' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800'}`} role="option" aria-selected={isSelected}>
                ...
                {hasMore && (
                    <li className="px-4 py-3">
                        <button 
                            ...
                            className="w-full py-2.5 mb-10 bg-slate-100 dark:bg-slate-800 hover:bg-primary/10 hover:text-primary rounded-xl text-xs font-black uppercase tracking-widest transition-all text-slate-500"
                        >
