...
function IngredientSections({name,language}:{name:string;language:Language}){
  ...
  useEffect(()=>{
    Promise.all([fetchFullClinical(name,''),getPregReference(name,'')]).then(([f,p])=>{
      setFull(f); setPreg(p); setLoading(false);
    });
  },[name]);

  const get=(k:string)=>{
    for(const src of[full,preg]){
      if(!src)continue;
...
const ClinicalReferencePage:React.FC<Props>=({scientificName,tradeName,language,onClose,medicine})=>{
  ...
  // Known combos that should NOT be split into separate sections
  ...
  return(
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2" style={{paddingBottom:'calc(env(safe-area-inset-bottom)+24px)'}}>
        ...
        {/* Multi-ingredient: show per-ingredient sections with same accordion style */}
        {!loading&&isMulti&&(
          <div className="space-y-3">
            {allIngredients.map((ing,idx)=>(
              <div key={ing} className="space-y-2">
                <p className="text-[11px] font-black uppercase tracking-widest text-teal-600 px-1">
                  {idx+1}. {ing}
                </p>
                <IngredientSections name={ing} language={language}/>
              </div>
            ))}
          </div>
        )}
