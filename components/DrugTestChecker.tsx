...
// ── مطابقة دقيقة: كلمة كاملة مش substring ─────────────────────────────────
...
const DrugTestChecker: React.FC<Props> = ({
  ...
  // ── الأدوية من قاعدة البيانات للمادة المختارة ──────────────────────────
  ...
  const handleBack = () => {
    // ارجع للقائمة بدون إغلاق الـ modal
    setCurrentView('home');
      ...
      >
        ...
        <div className="relative bg-gradient-to-r from-violet-500 to-purple-600 px-5 pt-5 pb-4 flex-shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={onClose}
              className="text-white/80 active:scale-90 flex-shrink-0"
            >
                ...
                {medicinesForActive.map((m, i) => (
                  <button
                    ...
                    onClick={e => {
                      e.stopPropagation();
                      onMedicineSelect?.(m);
                      // مش بنقفل الـ DrugTest — يفضل في الـ background
                    }}
