...
const DrugTestChecker: React.FC<Props> = ({
  ...
  // زرار الرجوع — لو في detail view ارجع للـ home، لو في home اغلق
  const handleBack = () => {
    if (currentView === 'active_detail') {
      setCurrentView('home');
  ...
  return (
    ...
    >
      ...
      {/* Sheet — ثابتة في 85% من الشاشة */}
      <div
        className="flex flex-col bg-white dark:bg-dark-card rounded-t-3xl shadow-2xl overflow-hidden"
        style={{
          height: '88vh',
      ...
      >
        ...
        <div className="relative bg-gradient-to-r from-violet-500 to-purple-600 px-5 pt-4 pb-4 flex-shrink-0">
          ...
          <div className="flex items-center gap-3 mt-1">
            <button
              onClick={handleBack}
              className="w-8 h-8 bg-white/20 rounded-xl flex items-center justify-center text-white active:scale-90 transition-transform flex-shrink-0"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7"/>
              </svg>
            </button>
            <div className="w-8 h-8 bg-white/20 rounded-xl flex items-center justify-center text-base flex-shrink-0">🧪</div>
            <div className="flex-1 min-w-0">
              ...
              <p className="text-white/70 text-[10px] mt-0.5">
                {currentView === 'active_detail'
                  ? (ar ? `${medicinesForActive.length} دواء في القاعدة` : `${medicinesForActive.length} drugs`)
                  : (ar ? 'ابحث بالاسم التجاري أو المادة الفعالة' : 'Search by trade name or active')}
              </p>
            </div>
            <button onClick={onClose} className="w-8 h-8 bg-white/20 rounded-xl flex items-center justify-center text-white active:scale-90 flex-shrink-0">
        ...
        {currentView === 'active_detail' && selectedActive && (
          <div className="flex-1 overflow-y-auto overscroll-contain">
            ...
            {medicinesForActive.length > 0 && (
              <>
                <div className="px-4 pt-4 pb-2">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    {ar ? `أدوية في القاعدة (${medicinesForActive.length})` : `In Database (${medicinesForActive.length})`}
                  </p>
                </div>
                <div className="divide-y divide-slate-100 dark:divide-slate-800 pb-4">
                  {medicinesForActive.map((m, i) => (
                    ...
                    >
                      ...
                      {parseFloat(String(m['Public price'])) > 0 && (
            ...
            <p className="text-[9px] text-slate-400 text-center pb-6 px-4 leading-relaxed">
              {ar ? '⚠️ للإرشاد فقط. تحتاج تأكيد بفحص GC-MS' : '⚠️ For reference only. Confirm with GC-MS testing'}
            </p>
          </div>
        )}
          ...
          <>
            ...
            <div className="px-4 py-3 border-b border-slate-100 dark:border-dark-border flex-shrink-0">
              <div className="relative">
                ...
                {query && (
                  <button onClick={() => { setQuery(''); setDebouncedQuery(''); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"/>
                    </svg>
                  </button>
