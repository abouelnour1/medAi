...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content }) => {
  ...
  const createMarkup = (htmlString: string) => {
    ...
    const withPrice = withLinks.replace(/(\d+\.?\d*)\s*(SAR|SR|ريال|ر.س)/gi, '<span class="text-orange-600 dark:text-orange-400 font-black px-1 py-0.5 bg-orange-50 dark:bg-orange-900/30 rounded-md border border-orange-100 dark:border-orange-800/30">$1 ر.س</span>');
