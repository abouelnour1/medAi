...
const StockTracker: React.FC<{ allMedicines: Medicine[]; t: TFunction; language: string }> = ({ allMedicines }) => {
  ...
  const updateStock = useCallback((id: string, qty: number, minAlert: number, note: string) => {
    if(!activeId) return;
    setFacilities(prev => prev.map(f => f.id===activeId ? { ...f, stock:{ ...f.stock, [id]:{ ...f.stock[id], quantity:qty, minAlert, note, lastUpdated:new Date().toISOString() } } } : f));
  }, [activeId]);
      ...
      {active && (
        <>
          ...
          <div className="bg-gradient-to-br from-violet-600 to-purple-700 rounded-3xl p-5 text-white shadow-xl shadow-violet-500/20">
            ...
            <button onClick={() => setShowAdd(true)} className="w-full flex items-center justify-center gap-2 py-3 bg-white/15 hover:bg-white/25 rounded-2xl font-black text-sm active:scale-95 border border-white/20">
