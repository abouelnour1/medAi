...
import { TFunction, User, Medicine, AppSettings, InsuranceDrug, Cosmetic, Notification as AppNotification } from '../../types';
...
import { db, FIREBASE_DISABLED, functions } from '../../firebase';
  ...
  const handleAddItem = () => {
      if (addItemType === 'medicine' || addItemType === 'supplement') {
          ...
          const newMed: Medicine = {
              ...
              imgIndex1: newItemData.imgIndex1 || '',
              imgIndex2: newItemData.imgIndex2 || '',
      ...
      return (
          <div className="w-full">
              ...
              <div className="flex items-center gap-2">
                  <div className="flex-grow">
                    {isManual ? (
                        <div className="relative animate-fade-in">
                            <input ref={inputRef} type="text" value={value} onChange={(e) => onChange(e.target.value)} className="w-full p-2 border-2 border-primary/50 rounded-xl dark:bg-slate-700 dark:border-primary/50 focus:outline-none" placeholder={`Type new ${label.toLowerCase()}...`} />
                            <button onClick={() => setIsManual(false)} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-red-500">✕</button>
                        </div>
                    ) : (
                  ...
                  {!isManual && (
                      <button type="button" onClick={() => { setIsManual(true); onChange(''); }} className="p-2.5 bg-primary/10 text-primary rounded-xl hover:bg-primary hover:text-white transition-colors flex-shrink-0 shadow-sm" title={t('addNew')}><div className="text-lg leading-none font-bold">+</div></button>
                  )}
              </div>
          </div>
      );
  ...
  const renderUsers = () => (
      <div className="space-y-4 animate-fade-in">
          <div className="relative">
              ...
              <input type="text" placeholder={t('searchUserPlaceholder')} className="pl-10 w-full p-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800" value={userSearchTerm} onChange={(e) => setUserSearchTerm(e.target.value)} />
          </div>
                      ...
                      {filteredUsers.map(user => (
                          <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                              ...
                              <td className="px-4 py-4 whitespace-nowrap text-center text-sm font-semibold">
                                  <span className={user.aiRequestCount >= (user.customAiLimit || appSettings.aiRequestLimit) ? 'text-red-500' : 'text-slate-600 dark:text-slate-400'}>{user.aiRequestCount || 0}</span>
                                  <span className="text-slate-300 dark:text-slate-600 mx-1">/</span>
                                  <span className="text-slate-400">{user.customAiLimit || appSettings.aiRequestLimit || 3}</span>
                              </td>
                              <td className="px-4 py-4 whitespace-nowrap text-right">
                                  <button onClick={() => handleEditUserClick(user)} className="text-primary hover:text-primary-dark font-bold text-xs px-3 py-1.5 bg-primary/10 rounded-lg transition-colors border border-primary/20">{t('manageUser')}</button>
                              </td>
                          </tr>
                      ))}
                  </tbody>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {(addItemType === 'medicine' || addItemType === 'supplement') && (
                    <>
                        ...
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('tradeName')} *</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600 focus:ring-2 focus:ring-primary outline-none" value={newItemData['Trade Name'] || ''} onChange={e => setNewItemData({...newItemData, 'Trade Name': e.target.value})} /></div>
                        <div className="col-span-1 sm:col-span-2"><SmartSelect label={t('scientificName')} value={newItemData['Scientific Name'] || ''} onChange={val => setNewItemData({...newItemData, 'Scientific Name': val})} options={uniqueScientificNames} /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('price')} *</label><input type="number" step="0.01" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600 focus:ring-2 focus:ring-primary outline-none" value={newItemData['Public price'] || ''} onChange={e => setNewItemData({...newItemData, 'Public price': e.target.value})} /></div>
                        ...
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">صورة الفهرس 1 (Firebase URL)</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData.imgIndex1 || ''} onChange={e => setNewItemData({...newItemData, imgIndex1: e.target.value})} placeholder="https://firebasestorage..." /></div>
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">صورة الفهرس 2 (Firebase URL)</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData.imgIndex2 || ''} onChange={e => setNewItemData({...newItemData, imgIndex2: e.target.value})} placeholder="https://firebasestorage..." /></div>
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('pillImage')} (Firebase URL)</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData.imgPill || ''} onChange={e => setNewItemData({...newItemData, imgPill: e.target.value})} placeholder="https://firebasestorage..." /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('pillShape')}</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData.pillShape || ''} onChange={e => setNewItemData({...newItemData, pillShape: e.target.value})} placeholder="Round, Oval..." /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('scored')}</label><select className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData.pillScored || ''} onChange={e => setNewItemData({...newItemData, pillScored: e.target.value})}><option value="">Unknown</option><option value="Yes">Yes</option><option value="No">No</option></select></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('markings')}</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData.pillMarkings || ''} onChange={e => setNewItemData({...newItemData, pillMarkings: e.target.value})} /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('taste')}</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData.liquidTaste || ''} onChange={e => setNewItemData({...newItemData, liquidTaste: e.target.value})} /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('liquidColor')}</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData.liquidColor || ''} onChange={e => setNewItemData({...newItemData, liquidColor: e.target.value})} /></div>
                        <div className="col-span-1 sm:col-span-2"> <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Physical Notes</label><textarea className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData.physicalNotes || ''} onChange={e => setNewItemData({...newItemData, physicalNotes: e.target.value})} rows={2} /></div>
                        <div className="col-span-1 sm:col-span-2 lg:col-span-3 xl:col-span-4"><SectionTitle title="Regulatory & Classification" /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('legalStatus')}</label><select className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['Legal Status'] || 'OTC'} onChange={e => setNewItemData({...newItemData, 'Legal Status': e.target.value})}><option value="OTC">OTC</option><option value="Prescription">Prescription</option></select></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('productControl')}</label><select className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['Product Control'] || 'Uncontrolled'} onChange={e => setNewItemData({...newItemData, 'Product Control': e.target.value})}><option value="Uncontrolled">{t('uncontrolled')}</option><option value="Controlled">{t('controlled')}</option><option value="Restricted">{t('restricted')}</option></select></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('registrationNumber')}</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['RegisterNumber'] || ''} onChange={e => setNewItemData({...newItemData, 'RegisterNumber': e.target.value})} /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('oldRegisterNumber')}</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['Old register Number'] || ''} onChange={e => setNewItemData({...newItemData, 'Old register Number': e.target.value})} /></div>
                        <div className="col-span-1 sm:col-span-2 lg:col-span-3 xl:col-span-4"><SectionTitle title="Manufacturer & Logistics" /></div>
                        <div className="col-span-1 sm:col-span-2"><SmartSelect label={t('manufacturer')} value={newItemData['Manufacture Name'] || ''} onChange={val => setNewItemData({...newItemData, 'Manufacture Name': val})} options={uniqueManufacturers} /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('countryOfManufacture')}</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['Manufacture Country'] || ''} onChange={e => setNewItemData({...newItemData, 'Manufacture Country': e.target.value})} /></div>
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('marketingCompany')}</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['Marketing Company'] || ''} onChange={e => setNewItemData({...newItemData, 'Marketing Company': e.target.value})} /></div>
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('mainAgent')}</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['Main Agent'] || ''} onChange={e => setNewItemData({...newItemData, 'Main Agent': e.target.value})} /></div>
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('agents')} (2nd / 3rd)</label><div className="flex gap-2"><input type="text" placeholder="Second Agent" className="w-1/2 p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['Secosnd Agent'] || ''} onChange={e => setNewItemData({...newItemData, 'Secosnd Agent': e.target.value})} /><input type="text" placeholder="Third Agent" className="w-1/2 p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['Third agent'] || ''} onChange={e => setNewItemData({...newItemData, 'Third agent': e.target.value})} /></div></div>
                        <div className="col-span-1 sm:col-span-2 lg:col-span-3 xl:col-span-4"><SectionTitle title="Storage & Codes" /></div>
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('storageConditions')} (En)</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['Storage conditions'] || ''} onChange={e => setNewItemData({...newItemData, 'Storage conditions': e.target.value})} /></div>
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('storageConditions')} (Ar)</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600 text-right" dir="rtl" value={newItemData['Storage Condition Arabic'] || ''} onChange={e => setNewItemData({...newItemData, 'Storage Condition Arabic': e.target.value})} /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('shelfLife')}</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['shelfLife'] || ''} onChange={e => setNewItemData({...newItemData, 'shelfLife': e.target.value})} /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('atcCode')}</label><div className="flex gap-2"><input type="text" placeholder="Code 1" className="w-1/2 p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['AtcCode1'] || ''} onChange={e => setNewItemData({...newItemData, 'AtcCode1': e.target.value})} /><input type="text" placeholder="Code 2" className="w-1/2 p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['AtcCode2'] || ''} onChange={e => setNewItemData({...newItemData, 'AtcCode2': e.target.value})} /></div></div>
                    </>
                )}
                {addItemType === 'cosmetic' && (
                    <>
                        ...
                        <div className="col-span-1 sm:col-span-2"><SmartSelect label={t('brandName')} value={newItemData['BrandName'] || ''} onChange={val => setNewItemData({...newItemData, 'BrandName': val})} options={uniqueBrands} /></div>
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('productName')} (En) *</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['SpecificName'] || ''} onChange={e => setNewItemData({...newItemData, 'SpecificName': e.target.value})} /></div>
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('productName')} (Ar)</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600 text-right" dir="rtl" value={newItemData['SpecificNameAr'] || ''} onChange={e => setNewItemData({...newItemData, 'SpecificNameAr': e.target.value})} /></div>
                        <div className="col-span-1 sm:col-span-2 lg:col-span-3 xl:col-span-4"><SectionTitle title="Categorization" /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">Category (En)</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['FirstSubCategoryEn'] || ''} onChange={e => setNewItemData({...newItemData, 'FirstSubCategoryEn': e.target.value})} /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">Category (Ar)</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600 text-right" dir="rtl" value={newItemData['FirstSubCategoryAr'] || ''} onChange={e => setNewItemData({...newItemData, 'FirstSubCategoryAr': e.target.value})} /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">Sub-Category (En)</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['SecondSubCategoryEn'] || ''} onChange={e => setNewItemData({...newItemData, 'SecondSubCategoryEn': e.target.value})} /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">Sub-Category (Ar)</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600 text-right" dir="rtl" value={newItemData['SecondSubCategoryAr'] || ''} onChange={e => setNewItemData({...newItemData, 'SecondSubCategoryAr': e.target.value})} /></div>
                        <div className="col-span-1 sm:col-span-2 lg:col-span-3 xl:col-span-4"><SectionTitle title="Origin" /></div>
                        <div className="col-span-1 sm:col-span-2"><SmartSelect label="Manufacturer Name (En)" value={newItemData['manufacturerNameEn'] || ''} onChange={val => setNewItemData({...newItemData, 'manufacturerNameEn': val})} options={uniqueManufacturers} /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">Country (En)</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['manufacturerCountryEn'] || ''} onChange={e => setNewItemData({...newItemData, 'manufacturerCountryEn': e.target.value})} /></div>
                        <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">Country (Ar)</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600 text-right" dir="rtl" value={newItemData['manufacturerCountryAr'] || ''} onChange={e => setNewItemData({...newItemData, 'manufacturerCountryAr': e.target.value})} /></div>
                        <div className="col-span-1 sm:col-span-2 lg:col-span-3 xl:col-span-4"><SectionTitle title="Composition" /></div>
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('quickActionIngredient')}</label><textarea className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" rows={2} value={newItemData['Active ingredient'] || ''} onChange={e => setNewItemData({...newItemData, 'Active ingredient': e.target.value})} /></div>
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">Key Ingredients</label><textarea className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" rows={2} value={newItemData['Key Ingredients'] || ''} onChange={e => setNewItemData({...newItemData, 'Key Ingredients': e.target.value})} /></div>
                        <div className="col-span-1 sm:col-span-2"><label className="block text-xs font-bold uppercase text-slate-500 mb-1">Highlights</label><input type="text" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" value={newItemData['Highlights'] || ''} onChange={e => setNewItemData({...newItemData, 'Highlights': e.target.value})} /></div>
                    </>
                )}
            </div>
          </div>
          <div className="pt-4 border-t dark:border-slate-700 flex justify-end flex-shrink-0">
              <button onClick={handleAddItem} className="px-6 py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-lg shadow-lg shadow-green-600/20 transition-all">{t('save')}</button>
          </div>
      </div>
  );
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm space-y-6 animate-fade-in">
          ...
          <div className="space-y-4">
              <div>
                  ...
                  <input type="text" value={bcTitle} onChange={e => setBcTitle(e.target.value)} className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" placeholder="Enter title..." />
              </div>
              <div>
                  <label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('notificationBody')}</label>
                  <textarea value={bcBody} onChange={e => setBcBody(e.target.value)} className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600" rows={4} placeholder="Type message here..." />
              </div>
              <div className="flex items-center gap-2 py-2">
                  <input type="checkbox" id="push-toggle" checked={isPushEnabled} onChange={e => setIsPushEnabled(e.target.checked)} className="w-5 h-5 accent-primary" />
                  <label htmlFor="push-toggle" className="text-sm font-bold text-slate-600 dark:text-slate-300">
          ...
          <div>
              ...
              <div className="grid grid-cols-1 gap-6">
                  ...
                  <div className="flex items-center">
                      <input id="ai-enabled" type="checkbox" checked={appSettings.isAiEnabled} onChange={(e) => setAppSettings({ ...appSettings, isAiEnabled: e.target.checked })} className="h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded" />
                      <label htmlFor="ai-enabled" className="ml-2 block text-sm text-slate-900 dark:text-white">Enable AI Features Globally</label>
                  </div>
                  <button onClick={() => { updateSettings(appSettings); alert(t('save')); }} className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">{t('save')}</button>
              </div>
          </div>
      </div>
  );
        ...
        {isEditUserModalOpen && editingUser && (
            <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setIsEditUserModalOpen(false)}>
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-2xl w-full max-w-lg border border-slate-100 dark:border-slate-700 flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                    <div className="flex justify-between items-start mb-6 border-b border-slate-100 dark:border-slate-700 pb-4 flex-shrink-0"><div><h3 className="text-xl font-bold">{t('manageUser')}</h3><p className="text-sm text-slate-500">{editingUser.email}</p></div><button onClick={() => setIsEditUserModalOpen(false)} className="text-gray-400 hover:text-gray-600"><span className="text-2xl">&times;</span></button></div>
                    <div className="space-y-6 overflow-y-auto flex-grow pr-2">
                        <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700"><h4 className="font-bold text-sm text-slate-700 dark:text-slate-300 mb-3">{t('role')}</h4><div className="flex gap-2"><button onClick={() => setEditingUser({...editingUser, role: 'premium'})} className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${editingUser.role === 'premium' ? 'bg-blue-600 text-white' : 'bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600'}`}>{t('premiumRole')}</button><button onClick={() => setEditingUser({...editingUser, role: 'admin'})} className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${editingUser.role === 'admin' ? 'bg-purple-600 text-white' : 'bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600'}`}>{t('adminRole')}</button></div></div>
                        <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700"><h4 className="font-bold text-sm text-slate-700 dark:text-slate-300 mb-3">{t('aiUsage')}</h4><div className="flex justify-between items-center mb-4 p-3 bg-white dark:bg-slate-800 rounded border dark:border-slate-700"><div><span className="text-xs text-slate-500">Current Usage</span><p className="font-bold text-xl">{editingUser.aiRequestCount || 0} / {editingUser.customAiLimit || appSettings.aiRequestLimit || 3}</p></div><button onClick={() => setEditingUser({...editingUser, aiRequestCount: 0})} className="text-xs font-bold text-blue-600 hover:text-blue-800 underline">{t('resetAiUsage')}</button></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-xs font-bold text-slate-500 mb-1">{t('customAiLimit')}</label><input type="number" value={editingUser.customAiLimit || ''} onChange={e => setEditingUser({...editingUser, customAiLimit: parseInt(e.target.value) || undefined})} placeholder="Default" className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600 focus:ring-2 focus:ring-primary outline-none" /></div><div className="flex flex-col justify-end"><label className="flex items-center cursor-pointer p-2 border rounded-lg bg-white dark:bg-slate-800 dark:border-slate-600"><input type="checkbox" className="mr-2" checked={editingUser.prescriptionPrivilege || false} onChange={e => setEditingUser({...editingUser, prescriptionPrivilege: e.target.checked})} /><span className="text-xs font-bold">{t('grantPrescriptionAccess')}</span></label></div></div></div>
                        <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-lg border border-red-100 dark:border-red-800"><h4 className="font-bold text-sm text-red-700 dark:text-red-400 mb-3">{t('dangerZone')}</h4><button onClick={() => handleUserDelete(editingUser.id)} className="w-full py-2 px-4 bg-white dark:bg-slate-800 border border-red-200 dark:border-red-700 text-red-600 dark:text-red-400 font-bold rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors flex items-center justify-center gap-2"><TrashIcon /> {t('delete')}</button></div>
                    </div>
                    <div className="flex justify-end gap-3 pt-6 border-t border-slate-100 dark:border-slate-700 mt-2"><button onClick={() => setIsEditUserModalOpen(false)} className="px-4 py-2 text-slate-600 dark:text-slate-300 font-medium hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg">{t('cancel')}</button><button onClick={handleSaveUser} className="px-6 py-2 bg-primary text-white font-bold rounded-lg hover:bg-primary-dark shadow-lg shadow-primary/30">{t('save')}</button></div>
                </div>
            </div>
        )}
    </div>
