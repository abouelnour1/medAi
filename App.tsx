...
const App: React.FC = () => {
  ...
  const renderContent = () => {
                  ...
                  {searchTerm.length === 0 && activeFiltersCount === 0 && (
                    <div className="mb-3">
                      <button
                        ...
                        className="w-full flex items-center justify-between px-3 py-2.5 mb-1 rounded-2xl active:scale-[0.98] transition-all"
                        style={{ background: quickToolsOpen ? '#f0faf7' : '#f8fafb', border: '1.5px solid', borderColor: quickToolsOpen ? '#b2e6da' : '#e5eae8' }}
                      >
                        <div className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-lg flex items-center justify-center" style={{ background: quickToolsOpen ? '#006a6018' : '#8a938f18' }}>
                            <svg className="w-3.5 h-3.5" style={{ color: quickToolsOpen ? '#006a60' : '#8a938f' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h16"/>
                            </svg>
                          </div>
                          <p className="text-[11px] font-black uppercase tracking-widest" style={{ color: quickToolsOpen ? '#006a60' : '#8a938f' }}>
                            {language === 'ar' ? 'الأدوات السريعة' : 'Quick Tools'}
                          </p>
                        </div>
                        <svg className={`w-3.5 h-3.5 transition-transform ${quickToolsOpen ? 'rotate-180' : ''}`} style={{ color: quickToolsOpen ? '#006a60' : '#8a938f' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
      ...
      if (activeTab === 'insurance') {
          ...
          return <InsuranceSearchView t={t} language={language} allMedicines={medicines} insuranceData={insuranceData} onSelectInsuranceData={(d) => { if (scrollContainerRef.current) scrollPositions.current.set('insuranceSearch', scrollContainerRef.current.scrollTop); setSelectedInsurance(d); setView('insuranceDetails'); if (scrollContainerRef.current) { scrollContainerRef.current.scrollTop = 0; } }} insuranceSearchTerm={insuranceSearchTerm} setInsuranceSearchTerm={setInsuranceSearchTerm} insuranceSearchMode={insuranceSearchMode} setInsuranceSearchMode={setInsuranceSearchMode} headerHeight={headerHeight} isKeyboardOpen={isKeyboardOpen} renderPart="results" />;
      }
  ...
  return (
    <div className="bg-slate-200 dark:bg-slate-950 min-h-full flex justify-center"><div
      className="bg-light-bg dark:bg-dark-bg text-slate-900 dark:text-slate-100 h-full flex flex-col overflow-hidden relative min-h-0 w-full max-w-[480px] shadow-2xl"
      onTouchStart={(e) => {
        const target = e.target as HTMLElement;
        // Only dismiss keyboard when tapping main scroll area, not header/buttons/search
        const isInteractive = target.tagName === 'INPUT' || target.tagName === 'TEXTAREA' || target.tagName === 'BUTTON' || target.tagName === 'SELECT' || target.closest('input,textarea,button,[role="button"],header,nav,[data-search-bar]');
        if (!isInteractive && isKeyboardOpen) {
